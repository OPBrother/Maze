...
class Maze(tk.Tk, object):
    def __init__(self):
        ...
        self.filename = '/home/syzx/Maze/lib/envs/maze.xml'
        self.actions, self.controllable, self.ncontrollable, self.states, self.terminal, self.initial_state, self.transitions = parse(
            self.filename)
        self.actual_state = self.initial_state
        # 定义RL状态与FSM状态的转换关系
        self.state_tran = np.ones([5, 5], dtype=type)
        for i in range(5):
            for j in range(5):
                self.state_tran[j][i] = [5 + 40 * i, 5 + 40 * j, 35 + 40 * i, 35 + 40 * j]
        self.state_tran = self.state_tran.reshape(1, 25)
    ...
    def reset(self):
        ...
        self.rect = self.canvas.create_rectangle(
            ...
            origin[0] + 15, origin[1] + 15,
            fill='red')
        # TODO  是否定义一个self变量 self.observation = self.canvas.coords(self.rect)
        # 然后转换出fsm的状态
    ...
    def step_fsm(self,action):
        s = self.canvas.coords(self.rect)
        # 得到fsm当前状态
        for i in range(25):
            if self.state_tran[0][i] == s:
                self.actual_state = i
                break
        # 得到fsm执行事件后新状态
        for trans in self.transitions:
            if (trans[0] == self.actual_state and trans[2] == action):
                ...
                break
        # 新状态对于s_
        s_ = self.state_tran[0][self.actual_state]
        # 在画布上移动位置
        base_action = np.array([0, 0])
        base_action[0] = s_[0]-s[0]
        base_action[1] = s_[1]-s[1]
        # 在画布上将探险者移动到下一位置
        self.canvas.move(self.rect, base_action[0], base_action[1])
        # 得到下一个s的位置
        # s_ = self.canvas.coords(self.rect)

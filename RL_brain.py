...
class RL(object):
    ...
    def choose_action_fsm(self, observation, state_tran, env_transitions):
        temp = json.loads(observation)
        # 得到fsm当前状态
        for i in range(25):
            if state_tran[0][i] == temp:
                # 得到fsm状态可能的动作，添加在actions_list中
                for trans in env_transitions:
                    if trans[0] == i:
                        self.actions_list.append(trans[2])
                break
        self.check_state_exist_fsm(observation)
        if np.random.rand() < self.epsilon:
            # 选择最优行为
            state_action = self.q_table.loc[observation, :]
            # 因为一个状态下最优行为可能会有多个,所以在碰到这种情况时,需要随机选择一个行为进行
